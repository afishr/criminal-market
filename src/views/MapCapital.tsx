...
import './MapCapital.css'
import data from "../json/septembrie_2019.json"

export const MapCapitalPage: React.FC = () => {
	...
	const handleClick = (event: any) => {
		let districtName = event.target.getAttribute('data-title');		
		
		const obj = getCriminalByDistrict(districtName);
		const lvl = obj.value / 10000;		

		event.target.setAttribute('fill', '#ff0000');
		event.target.setAttribute('fill-opacity', lvl * 100 + "%");

		const sortedObj = getTopFiveCategoriesByDistrict(obj.name);		

		setTopFive([sortedObj?.district.heads[0], sortedObj?.district.heads[1], sortedObj?.district.heads[2], sortedObj?.district.heads[3], sortedObj?.district.heads[4]]);	
		setDistrict(obj);
	...
	const getListOfDistrictsWithDetails = () => {
		return data.map(e => {
			return {
				...
				value: e.district.heads[0].value
			}
		})
	}

	const listItems = topFive.map((e: any) => {
		return <p>{e.name} - {e.value}</p>
	});
	...
	return <>
		...
		<div className="container-map">
			<svg 
				...
				height="768.87463">
			<g>
				<path fill="#dadff7" data-title="Buiucani" onClick={handleClick}
				...
				<path fill="#dadff7" data-title="RÎŞCANI" onClick={handleClick}
				...
				<path fill="#dadff7" data-title="Centru" onClick={handleClick}
				...
				<path fill="#dadff7" data-title="Ciocana" onClick={handleClick}
				...
				<g>
					<path  fill="#dadff7" data-title="Botanica" onClick={handleClick}
